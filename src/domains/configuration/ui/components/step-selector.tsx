...
import { courseRepository } from "@/domains/dance/data/repositories";
import { DanceCourse, DanceStep } from "@/domains/dance/domain/entities";
...
export function StepSelector({
  ...
  const isSomeCourseSelected = (courseId: string) => {
    const courseSteps = stepsByCourse[courseId] || [];
    return (
      courseSteps.some((step) => selectedSteps.includes(step.id)) &&
          ...
          <CardHeader>
            <div className="flex items-center space-x-2">
              <Checkbox
                id={`course-${course.id}`}
                checked={
                  isSomeCourseSelected(course.id)
                    ? "indeterminate"
                    : isAllCourseSelected(course.id)
                }
